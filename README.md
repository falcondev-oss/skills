...
| [`quick-implement-pr`](skills/quick-implement-pr/SKILL.md) | Implement a small piece of work which doesn't need a detailed spec, commit to a new branch and create a PR |
| [`implementation-review`](skills/implementation-review/SKILL.md) | Reviews completed implementation work with spec, code-quality and over-engineering review passes, then fixes and rechecks every finding. |
| [`standing-orders`](skills/standing-orders/SKILL.md) | The machine-wide defaults in force every conversation: taste, telegram-terse replies, the skills to reach for (`ponytail`, `conventional-commits`), UI verification in a subagent, shared-machine care — plus the house [coding standards](skills/standing-orders/standards/CODING_STANDARDS.md) it carries, which every code-style-aware skill reads. Wired into each harness's root config by `setup-falcondev-oss-skills`. |
...
| [`goodday-api`](skills/goodday-api/SKILL.md) | Calling the [goodday](https://www.goodday.work/developers/api-v2) work-management API v2 for Projects, Tasks, and Users. Complete, exhaustive route reference (list/read/create/update projects, folders, and tasks; comment, reply, tag, change status; read users and rate history), token read from `GOODDAY_API_TOKEN`. |
